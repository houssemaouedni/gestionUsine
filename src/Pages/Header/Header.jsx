...
import { setStatusMenu } from '../../Configs/action';
...
const Header = (props) => {
    ...
    const setActiveMenu = (status) => {

        dispatch(setStatusMenu(status));
      }
  ...
  return (
    ...
    <div className="shadow-sm">
        <div className="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-[#0e1726]">
            <div className={`${activeMenu ? 'hidden' : 'w-40 flex items-center justify-between'} `}>
                <div  className="main-logo flex shrink-0 items-center">
                    <img className="inline  " src={logo} alt="image" />
                    {/* <span className="hidden align-middle text-2xl font-semibold transition-all duration-300  md:inline">Vakay</span> */}
                </div>
                <button className='ml-4' onClick={() => setActiveMenu(!activeMenu)}>
